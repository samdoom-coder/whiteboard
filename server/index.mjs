 ...
 *
 * Run standalone with:  npm run server   (or: node server/index.mjs)
 * Or attach to any HTTP/HTTPS server via `createRelay(server)`.
 */
...
/**
 * Attach the collaboration relay to an existing HTTP/HTTPS server.
 * Returns the underlying WebSocketServer so callers can hook events if needed.
 */
